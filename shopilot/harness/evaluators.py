...
class Evaluator:
    ...
    def evaluate(self,run_id:str)->EvaluationReport:
        ...
        report=EvaluationReport(passed=not failures,checks=checks,metrics={"trace_events":float(len(trace)),"artifact_count":float(len(arts)),"retry_count":float(sum(e["event_type"]=="attempt" for e in trace)),"latency_ms":sum(e.get("latency_ms",0) for e in trace),"cost":sum(e.get("cost",0) for e in trace),"llm_judge_score":float(judge["score"]) if judge.get("score") is not None else 0.0},failures=failures)
